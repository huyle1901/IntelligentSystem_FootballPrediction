"""Analytics + lightweight auth store.

SQLite is used by default for local development. When DATABASE_URL is set, the
same store uses PostgreSQL, which is the default path for Docker Compose.
"""
...
from ..config import ALLOWED_ROLES, ANALYTICS_DB, DATABASE_URL

try:
    import psycopg
    from psycopg.rows import dict_row
except ImportError:  # pragma: no cover - optional for SQLite-only local runs.
    psycopg = None
    dict_row = None


class AnalyticsStore:
    def __init__(self, db_path: Path = ANALYTICS_DB, database_url: str = DATABASE_URL) -> None:
    ...
    def _ensure_column(self, conn: Any, table_name: str, column_name: str, column_sql: str) -> None:
        if self.use_postgres:
            row = conn.execute(
                """
                SELECT column_name
    ...
    @staticmethod
    def _public_user_payload(row: Any | None) -> dict[str, Any] | None:
        if not row:
            return None
        item = dict(row)
        return {
    ...
    def get_user_by_id(self, user_id: str | None) -> dict[str, Any] | None:
        ...
        with self._connect() as conn:
            row = conn.execute(
                self._q("SELECT user_id, username, display_name, role, created_at, last_seen_at FROM users WHERE user_id = ?"),
                (normalized,),
            ).fetchone()
    ...
    def register_user(
        ...
        with self._connect() as conn:
            existing = conn.execute(
                self._q("SELECT user_id FROM users WHERE username = ?"),
                (normalized_username,),
            ).fetchone()
            if existing:
    ...
    def authenticate_user(self, username: str, password: str) -> dict[str, Any] | None:
        ...
        with self._connect() as conn:
            ...
            now = self._now()
            conn.execute(self._q("UPDATE users SET last_seen_at = ? WHERE user_id = ?"), (now, row["user_id"]))
    ...
    def ensure_user(
        ...
        with self._connect() as conn:
            row = conn.execute(
                self._q("SELECT user_id, username, display_name, role FROM users WHERE user_id = ?"),
                (normalized,),
            ).fetchone()
    ...
    def track_team_view(self, league: str, team_name: str, user_id: str | None = None) -> None:
        ...
        with self._connect() as conn:
            conn.execute(
                self._q("INSERT INTO team_views (user_id, league, team_name, viewed_at) VALUES (?, ?, ?, ?)"),
    ...
    def track_player_view(
        ...
        with self._connect() as conn:
            conn.execute(
                self._q("INSERT INTO player_views (user_id, league, team_name, player_name, viewed_at) VALUES (?, ?, ?, ?, ?)"),
    ...
    def track_match_view(self, league: str, match_id: str, user_id: str | None = None) -> None:
        ...
        with self._connect() as conn:
            conn.execute(
                self._q("INSERT INTO match_views (user_id, league, match_id, viewed_at) VALUES (?, ?, ?, ?)"),
    ...
    def top_teams(self, limit: int = 10, user_id: str | None = None) -> list[dict[str, Any]]:
        ...
        with self._connect() as conn:
            rows = conn.execute(self._q(query), params).fetchall()
    ...
    def user_activity(self, user_id: str, limit: int = 10) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            ...
            team_views = conn.execute(
                self._q("SELECT COUNT(*) AS c FROM team_views WHERE user_id = ?"),
                (normalized,),
            ).fetchone()["c"]
            player_views = conn.execute(
                self._q("SELECT COUNT(*) AS c FROM player_views WHERE user_id = ?"),
                (normalized,),
            ).fetchone()["c"]
            match_views = conn.execute(
                self._q("SELECT COUNT(*) AS c FROM match_views WHERE user_id = ?"),
                (normalized,),
            ).fetchone()["c"]
